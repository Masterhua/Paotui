        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yuebiangengjil','导入导出')"
                           icon="el-icon-download"
                           href="http://47.250.81.47:8080/xiaoyuanfuwupingtai/upload/yuebiangengjilMuBan.xls"
                        >批量导入余额变更记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yuebiangengjil','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yuebiangengjilUploadSuccess"
                                :on-error="yuebiangengjilUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('yuebiangengjil','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入余额变更记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yuebiangengjil','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yuebiangengjil.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yuebiangengjilName"
                                   header-align="center"
                                   label="原因">
                        <template slot-scope="scope">
                            {{scope.row.yuebiangengjilName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yuebiangengjilNumber"
                                   header-align="center"
                                   label="涉及数量">
                        <template slot-scope="scope">
                            {{scope.row.yuebiangengjilNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jifenTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('yuebiangengjil','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yuebiangengjil','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('yuebiangengjil','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                yuebiangengjilName : null,
                yuebiangengjilNumber : null,
                jifenTypes : null,
                insertTime : null,
                createTime : null,
            },
            ...
            json_fields: {
                //级联表字段
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '原因': "yuebiangengjilName",
                     '涉及数量': "yuebiangengjilNumber",
                     '类型': "jifenTypes",
                     '创建时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yuebiangengjil',//当前表表名,
                        sumColum : 'yuebiangengjil_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yuebiangengjil_name',//分组字符串字段
                        // types : 'yuebiangengjil_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "yuebiangengjil_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['yuebiangengjilDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "yuebiangengjil/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            yuebiangengjilUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yuebiangengjil/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入余额变更记录数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            yuebiangengjilUploadError(data){
