        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shouhuodizhi','导入导出')"
                           icon="el-icon-download"
                           href="http://47.250.81.47:8080/xiaoyuanfuwupingtai/upload/shouhuodizhiMuBan.xls"
                        >批量导入收货地址数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shouhuodizhi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shouhuodizhiUploadSuccess"
                                :on-error="shouhuodizhiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shouhuodizhi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shouhuodizhi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shouhuodizhiDizhi"
                                   header-align="center"
                                   label="地址">
                        <template slot-scope="scope">
                            {{scope.row.shouhuodizhiDizhi}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shouhuodizhi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shouhuodizhi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('shouhuodizhi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                shouhuodizhiName : null,
                shouhuodizhiPhone : null,
                shouhuodizhiDizhi : null,
                insertTime : null,
                updateTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shouhuodizhi',//当前表表名,
                        sumColum : 'shouhuodizhi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shouhuodizhi_name',//分组字符串字段
                        // types : 'shouhuodizhi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "shouhuodizhi_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['shouhuodizhiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shouhuodizhi/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shouhuodizhiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shouhuodizhi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
